...
class RestSearchResult extends Component {
  ...
  calcCloserBetterGrade = (distance, rate, minDist, maxDist) => {
    if (this.state.closerBetterScale > -1) {
      ...
      let normalized_distance =
        (1 - (distance - minDist) / (maxDist - minDist)) * 100;
  ...
  sortByCloserBetter(rests) {
    ...
    rests.map(rest => {
      rest.cbGrade = this.calcCloserBetterGrade(
        rest.distance,
        rest.averageRate,
        minDist,
        maxDist
    ...
    return rests.sort((restA, restB) => {
      return restB.cbGrade - restA.cbGrade;
    });
  }
                    ...
                    <Col sm="3" key={_id}>
                      ...
                      >
                        <CardBody>
                          ...
                          <CardText>
                            {"Average Rate: " +
                              Math.round(averageRate * 100) / 100}
                          </CardText>
                          <CardText>
